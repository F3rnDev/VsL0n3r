...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	var noteType:Int;
	var noteLock:Bool;
	var check_lockNotes:FlxUICheckBox;
	...
	function addNoteUI():Void
	{
		...
		//noteType Stuff
		var noteTypeSelector = new FlxUINumericStepper(10, 30, 1, 0, 0, 10, 0);
		noteTypeSelector.value = noteType;
		noteTypeSelector.name = 'note_type';

		var noteTypeLabel = new FlxText(74, 30,'Note Type');
		
		check_lockNotes = new FlxUICheckBox(10, 50, null, null, 'Lock the Notes', 100);
		check_lockNotes.name = 'check_lockNotes';
		...
		tab_group_note.add(check_lockNotes);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			{
				if (nums.value >= 1) //When adding more noteTypes, change this if statement
					nums.value = 1;
	...
	override function update(elapsed:Float)
	{
		...
		curRenderedNotes.forEach(function(note:Note)
			{
				if (strumLine.overlaps(note) && pressArray[Math.floor(Math.abs(note.noteData))] && !noteLock)
				{
					deleteNote(note);
					delete = true;
					trace('deelte note');
		...
		{
			if (FlxG.mouse.overlaps(curRenderedNotes) && !noteLock)
			{
				curRenderedNotes.forEach(function(note:Note)
				{
